...
const NCMEC_REPLY =
  "We're here for you. You're not alone. We'll help you remove any images of you online and support you through this. Can you send us some info and evidence (e.g. screenshots) so we can help?";
...
function getStatsFriendCorrectReply(name) {
  return `Yes, you are correct ${name || "there"}, that is an interesting information isn't it!`;
}
...
function getThreadReply(pendingAnswer, currentScenario, playerName) {
  if (!pendingAnswer || !currentScenario)
    return {
      text: null,
      from: null,
      playerMessageOverride: null,
      switchedThreadPlayerMessage: null,
    };
  ...
  if (isCorrect) {
    ...
    if (type === "tell_adult")
      return {
        text: getTrustedAdultReply(playerName),
        from: "trusted_adult",
        playerMessageOverride: null,
        switchedThreadPlayerMessage: null,
      };
    if (currentScenario.type === "quiz")
      return {
        text: getStatsFriendCorrectReply(playerName),
        from: "friend",
        playerMessageOverride: null,
        switchedThreadPlayerMessage: null,
      };
    if (currentScenario.type === "helping_friend")
      return {
        text: getHelpingFriendCorrectReply(playerName),
        from: "friend",
        playerMessageOverride: null,
        switchedThreadPlayerMessage: null,
      };
  ...
  if (currentScenario.type === "helping_friend" && fb) {
    if (fb.incorrectSwitchToAdult && fb.ourMessageToAdult) {
      return {
        ...
        from: "trusted_adult",
        playerMessageOverride: null,
        switchedThreadPlayerMessage: fb.ourMessageToAdult,
      };
    }
    if (fb.ourThreadMessage) {
      return {
        text: fb.threadReply,
        from: "friend",
        playerMessageOverride: fb.ourThreadMessage,
        switchedThreadPlayerMessage: null,
      };
    }
  }
  ...
  if (currentScenario.type === "quiz") {
    ...
    return {
      text: getStatsFriendIncorrectReply(
        correctOption?.text ?? "the one you didn't pick",
      ),
      from: "friend",
      playerMessageOverride: null,
      switchedThreadPlayerMessage: null,
...
function GamePage() {
  ...
  const {
    ...
    from: threadReplyFrom,
    playerMessageOverride,
    switchedThreadPlayerMessage,
  } = getThreadReply(pendingAnswer, currentScenario, playerName);
  ...
  const onSelectAnswer = (optionId, optionText) => {
    ...
    const correctOptionId = currentScenario?.options?.find(
      (o) => o.correct,
    )?.id;
    ...
    const delayMs = isDelayedConsequenceScenario
      ? FEEDBACK_DELAY_MS_WITH_CONSEQUENCE
      : FEEDBACK_DELAY_MS_DEFAULT;
  ...
  return (
    <div className="bg-[#8B5CF6] min-h-screen min-w-screen flex flex-col items-center relative">
      ...
      {/* Mobile: abilities + clock on one row (left/right), then question below. Desktop (lg+): Abilities | Question | Clock in one row */}
      <div className="flex flex-col lg:flex-row w-full justify-center px-3 sm:px-4 lg:px-0 gap-6 lg:gap-[3.75rem] mt-6 md:mt-25">
        {/* On mobile this is a row (abilities left, clock right). On lg it becomes "contents" so Abilities & Clock are direct flex children and we use order for layout */}
        <div className="flex flex-row justify-between items-start w-full lg:contents">
        ...
        {/* Center column: take all space between Abilities and Clock on desktop so QuestionDisplay is full length */}
        <div className="lg:order-2 lg:flex-1 lg:min-w-0 w-full">
          <QuestionDisplay
            ...
            correctOptionId={
              currentScenario?.options?.find((o) => o.correct)?.id
            }
      ...
      {/* Mobile: padding. Desktop: original mt-10 only */}
      <div className="mt-6 lg:mt-10 px-3 sm:px-4 lg:px-0 w-full">
        <AnswersDisplay
